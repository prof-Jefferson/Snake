...
public class SnakeGameManager : MonoBehaviour
{
	...
	// Controla a velocidade de movimento da snake
	float GetMoveInterval()
	{
		int snakeSize = snake.segments.Count;
		
		if (snakeSize < 10)
			return 0.5f; // meio segundo por movimento
		else if (snakeSize < 20)
			return 0.25f; // um quarto de segundo por movimento			
	...
	IEnumerator SnakeMovementRoutine()
	{
		while (true)
		{
			...
			yield return new WaitForSeconds(GetMoveInterval());  // Delay para o próximo movimento
		}
	}
